					...
					$.getJSON(
						...
						function(data) {
							...
							if(data && typeof data === 'object' && data.hasOwnProperty('query') && data.query.hasOwnProperty('results') && data.query.results.hasOwnProperty('channel')) {
								...
								for(var i=1;i<10;i++){
									$this.find("span.revslider_weather_date_forecast_"+i).text(weather_info["item"]["forecast"][i]["date"]);
									$this.find("span.revslider_weather_day_forecast_"+i).text(weather_info["item"]["forecast"][i]["day"]);
									$this.find("span.revslider_weather_code_forecast_"+i).text(weather_info["item"]["forecast"][i]["code"]);
									$this.find("span.revslider_weather_high_forecast_"+i).text(weather_info["item"]["forecast"][i]["high"]);
									$this.find("span.revslider_weather_low_forecast_"+i).text(weather_info["item"]["forecast"][i]["low"]);
									$this.find("span.revslider_weather_alt_high_forecast_"+i).text(get_alt_temp(weather_info["units"]["temperature"],weather_info["item"]["forecast"][i]["high"]));
									$this.find("span.revslider_weather_alt_low_forecast_"+i).text(get_alt_temp(weather_info["units"]["temperature"],weather_info["item"]["forecast"][i]["low"]));
									$this.find("span.revslider_weather_thumbnail_forecast_"+i).text('https://s.yimg.com/zz/combo?a/i/us/nws/weather/gr/' + weather_info["item"]["forecast"][i]["code"] + 'ds.png');
									$this.find("span.revslider_weather_image_forecast_"+i).text('https://s.yimg.com/zz/combo?a/i/us/nws/weather/gr/' + weather_info["item"]["forecast"][i]["code"] + 'd.png');
									$this.find("span.revslider_weather_icon_forecast_"+i).html('<i class=\"revslider-weather-icon revslider-weather-icon-' + weather_info["item"]["forecast"][i]["code"] + '\"></i>');
									$this.find("span.revslider_weather_text_forecast_"+i).text(weather_info["item"]["forecast"][i]["text"]);
								}
							}
						}
					);
